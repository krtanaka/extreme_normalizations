...
p1 = ggplot(data = df, aes(x = Time, y = year_sum, color = source, group = source)) +
  ...
  annotate("segment", x = tipped_cobe[,6], xend = tipped_cobe[,6], y = -0.08, yend = -0.11, color = cbPalette[1]) +
  annotate("segment", x = tipped_hadi[,6], xend = tipped_hadi[,6], y = -0.05, yend = -0.08, color = cbPalette[2]) +
  annotate("segment", x = tipped_er[,6], xend = tipped_er[,6], y = -0.02, yend = -0.05, color = cbPalette[3]) + 
  
  annotate("segment", x = tipped_cobe[1,6], xend = tipped_cobe[1,6], y = 0.55, yend = 0.75, color = cbPalette[1]) +
  annotate("segment", x = tipped_hadi[1,6], xend = tipped_hadi[1,6], y = 0.65, yend = 0.85, color = cbPalette[2]) +
  annotate("segment", x = tipped_er[1,6], xend = tipped_er[1,6], y = 0.75, yend = 0.95, color = cbPalette[3]) +
  ...
  geom_text(data = tipped_cobe[1,], aes(x = Time, label = paste0(Year, "-", Month), y = 0.75), 
            colour = cbPalette[1], angle = 0, hjust = 1.1, text = element_text(size = 11)) +
  geom_text(data = tipped_hadi[1,], aes(x = Time, label = paste0(Year, "-", Month), y = 0.85), 
            colour = cbPalette[2], angle = 0, hjust = 1.1,text = element_text(size = 11)) +
  geom_text(data = tipped_er[1,], aes(x = Time, label = paste0(Year, "-", Month), y = 0.95), 
            colour = cbPalette[3], angle = 0, hjust = 1.1,text = element_text(size = 11)) +
...
pdf(paste0("~/Desktop/Time_Series_", p, ".pdf"), height = 5, width = 10)
p1
dev.off()