...
setwd("/Users/ktanaka/extreme_normalizations/")
...
rcp %>% ggplot(aes(x, y, fill = sst)) + geom_raster(interpolate = T) + scale_fill_viridis_c() + coord_fixed() + theme_minimal()
...
r2 = r %>% select(x, y, sum) %>% mutate(t = "historical extremes (1980-2019)", z = sum) %>% select(x, y, z, t)
r3 = r %>% select(x, y, past_future) %>% mutate(t = "extreme + anomalies", z = past_future) %>% select(x, y, z, t)
...
r$t = factor(r$t, levels = c("historical extremes (1980-2019)", "rcp8.5 SST anomalies (2006-2099)",  "extreme + anomalies"))
